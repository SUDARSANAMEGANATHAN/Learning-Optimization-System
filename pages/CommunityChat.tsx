...
const CommunityChat: React.FC = () => {
  ...
  useEffect(() => {
    // Initialize socket connection (supports remote backend via VITE_API_BASE_URL)
    socketRef.current = io(apiBaseUrl || undefined);
  ...
  const handleSend = async () => {
    ...
    if (
      ...
      input.toLowerCase().includes("bot")
    ) {
      setIsAiTyping(true);
      try {
        const history = messages
          .slice(-5)
          .map((m) => ({ userName: m.userName, content: m.content }));
        const aiResponse = await geminiService.generateCommunityResponse(
          currentInput,
          history,
        );
        ...
        const aiMessage: CommunityMessage = {
          ...
          content:
            aiResponse || "I'm listening! How can I help with your studies?",
          timestamp: Date.now(),
        };
  ...
  const renderFileContent = (
      ...
      return (
        <div className="mt-3 rounded-2xl overflow-hidden border border-black/5 shadow-sm group/img relative">
          <img
            src={file.url}
            alt={file.name}
            className="max-w-full max-h-80 object-contain bg-gray-50/50"
    ...
    return (
      <div
        className={`mt-3 flex items-center gap-4 p-4 rounded-2xl border shadow-sm group/file transition-all ${
          isMe
      ...
      >
        <div
          className={`p-3 rounded-xl ${isPdf ? "bg-red-500/10 text-red-500" : "bg-blue-500/10 text-blue-500"}`}
        >
          {isPdf ? <FileText size={24} /> : <FileIcon size={24} />}
        </div>
        <div className="flex-1 min-w-0">
          <p
            className={`text-sm font-bold truncate ${isMe ? "text-white" : "text-slate-900"}`}
          >
            {file.name}
          </p>
          <p
            className={`text-[10px] font-black uppercase tracking-widest opacity-60 ${isMe ? "text-white/70" : "text-slate-400"}`}
          >
        ...
        <a
          href={file.url}
          download={file.name}
          className={`p-2 rounded-xl shadow-sm border transition-all hover:scale-110 ${
            isMe
              ? "bg-white/20 border-white/10 text-white"
              : "bg-white border-gray-100 text-slate-400 hover:text-green-600"
          }`}
        >
          <Download size={20} />
        </a>
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-800 p-8 text-white flex items-center justify-between shrink-0 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-green-100 text-xs font-black uppercase tracking-[0.2em]">
              Real-time Academic Network
            </p>
          </div>
        </div>
        ...
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40 grayscale pointer-events-none">
            ...
            <p className="text-lg font-medium text-gray-400">
              Introduce yourself to your fellow scholars.
            </p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`flex gap-5 group ${isMe ? "flex-row-reverse" : "flex-row"} animate-in fade-in slide-in-from-bottom-2`}
              >
                ...
                <div
                  className={`max-w-[75%] space-y-2 ${isMe ? "text-right" : "text-left"}`}
                >
                  <div
                    className={`flex items-center gap-3 mb-1 ${isMe ? "flex-row-reverse" : "flex-row"}`}
                  >
                    <span
                      className={`text-xs font-black uppercase tracking-widest flex items-center gap-1.5 ${isBot ? "text-indigo-600" : "text-gray-400"}`}
                    >
                      {isBot && <ShieldCheck size={14} />}
                      {msg.userName}
                      {isBot && (
                        <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[8px]">
                          VERIFIED BOT
                        </span>
                      )}
                    </span>
                  ...
                  <div
                    className={`px-6 py-4 rounded-[1.5rem] shadow-sm text-base font-semibold leading-relaxed ${
                      isMe
        ...
        {selectedFile && (
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-in slide-in-from-bottom-4">
            ...
            <button
              ...
              className="p-2 bg-white rounded-xl shadow-sm border border-gray-100 text-gray-400 hover:text-red-500 hover:scale-110 transition-all"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 relative group flex items-center">
            ...
            <input
              ...
              className={`w-full border-2 pl-16 pr-16 py-5 rounded-[1.5rem] outline-none transition-all font-bold text-gray-800 placeholder:text-gray-300 shadow-sm ${
                isAiActive
                  ? "bg-indigo-50 border-indigo-200 focus:border-indigo-600 focus:bg-white"
                  : "bg-gray-50 border-transparent focus:border-green-600 focus:bg-white"
              }`}
            />
            <button
              ...
              className={`absolute right-6 top-1/2 -translate-y-1/2 p-2 rounded-xl transition-all ${
                isAiActive
                  ...
                  : "bg-white text-gray-300 hover:text-indigo-600 hover:bg-indigo-50 border border-gray-100"
              }`}
